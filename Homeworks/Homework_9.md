...
Develop a **Bash** function named **ParallelWorlds**, which does the following:  
   ...
   4. Sends the execution for each chunk in the background in parallel using subshells. Each subshell sums up the integers in its chunk and stores the final sum for its chunk in some temporary file   
...
This will dump only lines 1-10 (both ends included) from ```toyData.dat``` into ```chunk1.dat```, and so on. Both the lower and upper ends can be supplied to **sed** programmatically via some **Bash** variables. Alternatively, and when chunks are meant to be of equal size, one can automate the splitting procedure with the **split** command. This is particularly handy if the content of the initial file, which needs to be fragmented, is line-oriented, like in this example. In that case, this example code snippet can be used:
...
because the operating system will allow only one command to make a new directory named 'someDir'. 
...
Given these two functions, the schematic usage of the lock mechanism is as follows, when multiple parallel processes are accessing and modifying the same file named 'someFile':
