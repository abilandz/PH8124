...
In the above syntax, "m" terminates a special sequence of characters starting with "\E[". The first two entries between "\E[" and "m", e.g. "1;31" in the example above, determine foreground color. The 3rd entry determines the background color, where background colors are labeled in the same way as the foreground, just starting from 40 instead of 30. The particular choice "48" for the background color gives the same shade as the terminal window. Finally, the command ```tput sgr0``` restores the default settings in the terminal.
