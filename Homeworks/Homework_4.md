...
2. terminates with the error exit status 2 if some file that the user has supplied does not exist (and it prints then the warning and the name of that problematic file)
...
**Challenge #2**: As a starting point for this homework, dump the following lines (including the empty lines!) in the file named `scrambled.txt`:
...
2. parses line-by-line through the file content, in each line counts the number of words, and replaces all occurrences of multiple empty characters with a single empty character
...
**Hint #1:** To parse line-by-line programmatically through the content of ASCII files, you can use **while+read** construct.
...
Operator `1>` redirects only the successful command output into some file.

**Hint #4**: The command **echo** preserves the multiple empty characters only if the arguments supplied to it are embedded within the quotes. Without quotes, one or more empty characters become one empty character in the **echo** printout.
...
2. If the user has replied 'Y', it prints `OK, continuing!`, and executes some subsequent code in the script
